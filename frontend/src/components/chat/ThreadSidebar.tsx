...
export function ThreadSidebar({
  ...
  return (
    <aside className="flex flex-col w-64 min-w-64 bg-gray-50 border-r border-gray-200 h-full dark:bg-slate-900 dark:border-slate-800">
      {/* Header */}
      <div className="p-4 border-b border-gray-200 dark:border-slate-800">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="font-semibold text-gray-900 dark:text-white text-sm">Amzur AI</span>
        </div>
        <button
          onClick={onCreate}
      ...
      <div className="px-3 pb-2 pt-3">
        <div className="mb-2 px-2 text-[11px] font-semibold uppercase tracking-[0.18em] text-gray-400 dark:text-slate-400">
          Workspace
        </div>
      </div>
      ...
      <nav className="flex-1 overflow-y-auto py-2">
        {threads.length === 0 ? (
          <p className="text-gray-400 text-xs text-center mt-8 px-4 dark:text-slate-500">No conversations yet. Start a new chat!</p>
        ) : (
          threads.map((thread) => (
            <ThreadItem
              key={thread.id}
      ...
      {/* Tools section */}
      <div className="px-3 pt-4 pb-2 border-t border-gray-200 dark:border-slate-800">
        <div className="mb-2 px-2 text-[11px] font-semibold uppercase tracking-[0.18em] text-gray-400 dark:text-slate-400">
          Tools
        </div>
          ...
          {(['chat','pdf','database','excel','generate','research','tictactoe'] as ToolKey[]).map((tool) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 rounded-lg px-3 py-2 text-sm font-bold transition-colors ${meta.color} ${activeTool === tool ? 'bg-slate-100 dark:bg-slate-800' : 'hover:bg-slate-50 dark:hover:bg-slate-800'}`}
      ...
      <div className="p-3 border-t border-gray-200 dark:border-slate-800">
        <div className="flex items-center gap-2">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm text-gray-900 dark:text-white truncate font-medium">{user.display_name ?? user.email}</p>
            <p className="text-xs text-gray-400 dark:text-slate-400 truncate">{user.email}</p>
          </div>
          <button onClick={onLogout} className="text-gray-400 hover:text-gray-600 dark:text-slate-400 dark:hover:text-white transition-colors shrink-0" title="Sign out">
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
